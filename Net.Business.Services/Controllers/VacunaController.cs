...
namespace Net.Business.Services.Controllers
{
    ...
    public class VacunaController : ControllerBase
    {
        ...
        public VacunaController(IRepositoryWrapper repository)
        {
        ...
        public async Task<IActionResult> GetAll([FromQuery] DtoFindVacuna value)
        {

            var objectGetAll = await _repository.Vacuna.GetAll(value.RetornaVacuna());
        ...
        /// <response code="200">Devuelve el listado completo </response>
        /// <response code="404">Si no existen datos</response>  
        [HttpGet("{id}", Name = "GetByIdVacuna")]
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        [ProducesDefaultResponseType]
        public async Task<IActionResult> GetbyId(int id)
        {
            var objectGetById = await _repository.Vacuna.GetById(new DtoFindVacuna { IdVacuna = id }.RetornaVacuna());
        ...
        public async Task<IActionResult> Create([FromBody] DtoInsertVacuna value)
        {
            if (value == null)
            {
            ...
            int ObjectNew = await _repository.Vacuna.Create(value.RetornaVacuna());

            if (ObjectNew == 0)
            {
                ModelState.AddModelError("", $"Algo salio mal guardando el registro {value.DescripcionVacuna}");
                return StatusCode(500, ModelState);
            }

            return CreatedAtRoute("GetByIdVacuna", new { id = ObjectNew }, ObjectNew);
        }
        /// <summary>
        ...
        public async Task<IActionResult> Update([FromBody] DtoUpdateVacuna value)
        {
            if (value == null)
            {
            ...
            await _repository.Vacuna.Update(value.RetornaVacuna());
        ...
        public async Task<IActionResult> Delete([FromBody] DtoDeleteVacuna value)
        {
            if (value == null)
            {
                return BadRequest(ModelState);
            ...
            await _repository.Vacuna.Delete(value.RetornaVacuna());
