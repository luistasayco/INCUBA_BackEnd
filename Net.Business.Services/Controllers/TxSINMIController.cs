        ...
        public async Task<IActionResult> Create([FromBody] DtoInsertTxSINMI value)
        {
            ...
            else
            {
                if (value.FlgCerrado != null)
                {
                    ...
                    {
                        var updateStatus = new DtoUpdateStatusTxSINMI { IdSINMI = ObjectNew.IdRegistro, IdUsuarioCierre = int.Parse(value.RegUsuario.ToString()), RegUsuario = value.RegUsuario, RegEstacion = value.RegEstacion };
